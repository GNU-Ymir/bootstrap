...
extern "C" void __bootstrap__set_executable_ (YrtArray name);
extern "C" void __bootstrap__activate_debug_ ();
extern "C" void __bootstrap__activate_verbose_ ();
extern "C" void __bootstrap__activate_standalone_ ();
extern "C" void __bootstrap__activate_dump_doc_ ();
extern "C" void __bootstrap__activate_version_ (YrtArray version);
...
{
    __bootstrap__set_executable_ (YrtArray {strlen (decoded_options [0].arg), (const void*)decoded_options [0].arg});
    for (unsigned int i = 0 ; i < argc ; i++) {
	switch (decoded_options [i].opt_index) {
	case OPT_g :
	case OPT_ggdb :
	    __bootstrap__activate_debug_ ();
	    __bootstrap__activate_version_ (YrtArray {5, (const void*) DEBUG_VERSION});
	    break;
	case OPT_v : __bootstrap__activate_verbose_ (); break;
	case OPT_nostdinc : __bootstrap__activate_standalone_ (); break;
	}
    }
		   ...
		   const struct cl_option_handlers *handlers ATTRIBUTE_UNUSED)
{
    ...
    if (code == OPT_I) {
    	// Add include dir
	__bootstrap__add_include_dir_ (YrtArray {strlen (arg), (const void*) arg});	
    } else if (code == OPT_iprefix) {
	__bootstrap__set_prefix_ (YrtArray {strlen (arg), (const void*) arg});	
    } else if (code == OPT_v) {
	__bootstrap__activate_verbose_ ();
	return false;
    } else if (code == OPT_nostdinc)  {
	__bootstrap__activate_standalone_ ();
    } else if (code == OPT_fdoc) {
	__bootstrap__activate_dump_doc_ ();	
    } else if (code == OPT_fversion_) {
	__bootstrap__activate_version_ (YrtArray {strlen (arg), (const void*) arg});	
