...
| [E3040](E3040.md) | `MACRO_NOT_RESTRICTABLE` | macro rule <> cannot be restricted to a list of values, only a built-in rule can be |
...
| [E4317](E4317.md) | `UNION_NOT_IMPL` | union <> does not implement trait <> |
...
These codes named messages that have since been deleted from the compiler, or
that are now raised only as a note under another diagnostic and carry no code
of their own. Nothing raises them today. They are listed so a code printed by an
older compiler can still be looked up, and so neither the code nor the name is
ever reused.
