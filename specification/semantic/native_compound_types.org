...
Arrays are a list of values of the same type stored in contiguous memory. Array
types have a size known at compilation time, and are stored locally (e.g. in the
stack if declared in a simple stack variable). The syntax to describe an array
type is the following ~[T ; N]~, with ~T~ a type and ~N~ an integer literal
describing the size of the array.

** Literals

An array literal can be written using two different syntaxes :

1. The first syntax is a list of values enclosed within brackets. In that case it has to be clear that the literal will be used to create an array type, for example by using the type of the left operand in an affectation expression, or using the type of the parameter in a function call.
...
2. The second syntax is close to the syntax of the type ~[V ; N]~ where ~V~ is a value and ~N~ is an integer literal. The value ~V~ is replicated (simple ~copy~) on each index of the table.
...
Array mutability depends on the mutability of the inner type contained in the
array. If the data inside the array are not borrowed, then array is not
borrowed, as the array datas are located at the array declaration location.
Every index of the array are replicated during /data movements/.

If inner datas are borrowing data (e.g. deeply mutable pointer type), then
mutability of array type is important and follows the same logic as any
borrowing types.
...
let dmut c = b ;  // not allowed, it would discard the const property of 'a'
\end{code}

** Properties

Properties on array types can be accessed using the operator ~::~ on a type expression. The properties are the following :

|----------+------------------------------------------------------------+-------------|
| name     | value                                                      | type        |
|----------+------------------------------------------------------------+-------------|
|----------+------------------------------------------------------------+-------------|
| ~size~   | The size of the array type in number of contained elements | ~usize~     |
|----------+------------------------------------------------------------+-------------|
| ~init~   | The initial value of the array, where every inner field    | ~typeof(x)~ |
|          | are set to ~T::init~                                       |             |
|          | -- example: ~[i32 ; 2]::init == [i32::init ; 2]~           |             |
|----------+------------------------------------------------------------+-------------|
...
There are no cast allowed on array types.

** Binary operators

Binary operators are divided in 3 groups :

- Concatenation :

Array can be concatenated if their inner type is the same using the operator
~~~. Mutability level of the generated array type is the maximum between the
lowest of the two operands, and ~2~. This operator is not commutative.
