...
A array is a compound type containing a list of values of the same type stored
...
important if the inner type of the array is a type borrowing data.
...
|          | where ~T~ is the inner type of the array                         |             |
...
- Concatenation :
  ...
  math operation, as ~+~ preferably refer to an addition of all the inner
...
- Comparison :
  ...
  Binary comparison operators are usable using two array with the same inner
