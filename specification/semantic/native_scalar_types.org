...
- Math :
  ...
  Binary math operators are usable using two float values of strictly the same
  type. The result of the operation takes the type of the operands. Usable
  operators are described in the following table. Power operator ~^^~ can take
  an integer value as right operand, instead of a ~float~ value, but can also
  take a ~float~ value.
