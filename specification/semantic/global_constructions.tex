...
\notebox {
  The symbol \texttt{qux::baz::funcInBaz} has access to the symbol
...
\notebox { \textit{UCS} was introduced to perform chained operations. It can be
  ...
  usage is map reduce in the form \texttt{([1, 2, 3]).map!\{|x| => x + 1\} ().reduce!\{|x, y| => x + y\} ().}
}
...
Figure~\ref{fig:example_mut_params} presents the state of the Stack and Heap
memory when calling the function \texttt{foo} at line 6. One can note that the
value of \texttt{x} borrows the same value as \texttt{a} from the \texttt{main}
...
\tipbox {
  In the main function, the call to \texttt{eprintln} has the same prototype
  as the function \texttt{println}, but write to stderr instead of stdout.
}
...
\cautionbox { Exceptions should primarily be used for error handling, not for
  ...
  representing the absence of a value in such scenarios, promoting clearer code.
}
...
\importantbox{ This is not perfect, because a function that is not marked as
  ...
  (e.g., in the previous code, the call to \texttt{baz} is considered safe, but
...
The \texttt{main} function is mandatory to create a runnable program, in the
