...
Tuple literal values may contain literal values that can be implicitely converted during compilation (e.g. integer literals, array literals, class objects, etc.). In this case, the conversion is performed at \texttt{cte} implicitely.
...
Iteration is never by reference, to modify a value inside a slice, the index
...
The \texttt{set} field in the \texttt{OptionType} definition describes the
...
taken into account during pattern matching and field access.
...
\item Access: The \texttt{.} operator is used to access the fields of the option
  type. There are three fields that can be accessed within an option value,
  which are described in the following table. Access to the \texttt{value} and
  \texttt{error} fields is done within a runtime check to make sure they are
  initialized. If they are not, a panic is triggered. For the \texttt{value}
  field to have a value, the \texttt{hasValue} field must be \texttt{true}, and for
  the \texttt{error} field to have a value, the \texttt{hasValue} field must be false
  and \texttt{hasError} must be true. There is no check when accessing the
  \texttt{hasValue} and \texttt{hasError} fields, which always have a value. These
  fields cannot be true at the same time, so $hasError \implies \lnot hasValue$ and
  $hasValue \implies \lnot hasError$, but $\lnot hasError \land \lnot hasValue$ can be
  true.
  ...
  \begin{center}\begin{adjustbox}{max width=1.0\linewidth}
      \begin{threeparttable}
        \begin{tabular}{|c|ll|}
          ...
          \texttt{hasError} & true iif the option has no value & \texttt{bool} \\
          & but has an error &  \\
          \Xhline{0.001pt}
          \texttt{value} & The value contained in the option value & \texttt{S}$^{1^{\phantom{j}}}$ \\
          \texttt{error} & The error contained in the option value & \texttt{E}$^{2^{\phantom{j}}}$ \\
          \hline
        \end{tabular}
        \begin{tablenotes}
        \item[1.] \texttt{\_\_pragma!inner (typeof (x), 0)}
        \item[2.] \texttt{\&Exception}
        \end{tablenotes}
      \end{threeparttable}
  \end{adjustbox}\end{center}
