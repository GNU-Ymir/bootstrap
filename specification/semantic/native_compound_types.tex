...
Tuple literal values may contain literal values that can be implicitely converted during compilation (e.g. integer literals, array literals, class objects, etc.). In this case, the conversion is performed at \token{cte} implicitely.
...
reference to the value is made through the variable \token{\#\_value}. The idea
is that you could iterate through a construction value, such as a function call (ex: \token{for i in foo ()}),
