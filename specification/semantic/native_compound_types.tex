...
\item Access: The \texttt{.} operator is used to access the fields of the option
  type. There are two fields that can be accessed within an option value, which
...
assert (!a.hasError && a.hasValue);
  ...
  The \texttt{value} and \texttt{error} fields are special fields that cannot be
  ...
  field \texttt{value} is accessed, there are two cases: 1) or the option has an
  error and in this case this error is thrown, 2) an exception of type
  \texttt{core::exception::EmptyOption} is thrown. This exception is of course
  catchable and rethrowable like any other exception. It was chosen not to make
  the program panic, to force developers to deal with empty option values, and
  to prevent the ``nullpointer exception'' effect.

  On the other hand, a runtime check is performed when the \texttt{field} field
  is accessed. If the option has no error the exception
  \texttt{core::exception::EmptyErrOption} is thrown.
