...
assert (!a.hasError && a.hasValue);
  ...
  The value of the option is not accessible this way to make sure there is a
  value inside the option. We could use a runtime check to panic if the value is
  not present, but we find it preferable to handle the none case explicitly to
  avoid implicit panics.
