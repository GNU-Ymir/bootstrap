...
\item \textit{Data borrowing} -- A data borrowing value is a value that points
  ...
  In the above example, both variables \texttt{x} and \texttt{y} borrow the data
...
index operation \texttt{(\&a)[7]} is strictly transformed into \texttt{*(\&a +
  (7us * sizeof (typeof(\&a)::inner)))}. This operation is unsafe because it
...
Range are useful for iteration or for accessing a subset of values (for example,
a subset of a slice).
...
\item Access: The operator \texttt{.} is utilized to access the field of the
  ...
  \begin{center}\begin{adjustbox}{max width=\linewidth}
    \begin{threeparttable}
      ...
      \end{tabular}
      \begin{tablenotes}
      \item[1.]\small \textit{Signed version of inner type for integer type
        ranges, an integer type for character type ranges, and
        \texttt{typeof(x)::inner} for float type ranges.}
      \end{tablenotes}
    \end{threeparttable}
\end{adjustbox}\end{center}
...
\item Access: The \texttt{.} operator is used to access fields that describe the
  array type. The right operand is the name of the field to be accessed. These
...
\item Concatenation: The concatenation operator \texttt{\~} is used to create an
  ...
  inner values of two arrays, rather than their concatenation.
...
\item Comparison:Binary comparison operators can be used with two arrays of the
  ...
  order is used, so the size of an array is only used if one of the two operands
...
\item Affect: An array value can be an lvalue if and only if it is
  mutable.Information about inner type mutability has already been discussed in
  the section on mutability, and will therefore not be discussed further here.
  The size of the left and right operands must of course be strictly equal.
...
let b : [i32] = copy a [0 .. 2];
  ...
  value of the range must be less than the second value, and must not be an
  including range. This is also checked at compile time if possible. If the
  values of the range are not known at compile time, then these checks are done
  at runtime. The contain field of the range has no effect on the array access,
  and the \texttt{scd} value is always considered not contained.

  If the range used to index the array is known at compile time, a new array
  value can be created. As with array literal, the creation of a slice or an
  array value is determined by the type of the left operand side, if the type is
  not defined, a slice is always created.
...
This is not the case for slices, so the \texttt{expand} operator cannot be used
on them. However, an array can be created from a slice value by using an index
operation with a range value as the right operand, the expand operator can force
the creation of an array value instead of a slice value. This is discussed
further in the next section on slices.
...
let a = copy [1, 2, 3]; // slice value
let b : [i32 ; 3] = [1, 2, 3]; // array value

foo (expand a [1 .. 2]); // create an array value from a slice value
foo (expand b [1 .. 2]); // create an array value from an array value
\end{lstlisting}
...
\texttt{T} is the inner type of the array. Unlike array types, slices are
...
slice may contain no data, in which case both the pointer and the size are
...
and restricting their operations in unsafe contexts would be extremely limiting.
...
explicit specification. The keywords \texttt{alias}, \texttt{copy} and
