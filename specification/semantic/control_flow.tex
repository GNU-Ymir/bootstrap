...
The keyword \token{lazy} can only decorate a variable defined with the keyword
\token{let} and that is not part of a pattern declaration. This means that an
iterator, a variable within a pattern in pattern matching, and similar
...
\notebox{ As discussed in Section~\ref{sec:lazy_param} and
  Section~\ref{sec:global_variable}, laziness can be applied to various kinds of
  constructs. In all these cases, laziness refers to the same behavior: the
  value is constructed only when it is needed.
